...
##' @return
##' @author Penergy
sem=function (model = NULL, data = NULL, meanstructure = "default",
    ...
    group.w.free = FALSE, cluster = NULL, constraints = "", estimator = "default",
    ...
    WLS.V = NULL, NACOV = NULL, zero.add = "default", zero.keep.margins = "default",
...
function (model = NULL, data = NULL, model.type = "sem", meanstructure = "default",
    ...
    verbose = FALSE, warn = TRUE, debug = FALSE)
{
    ...
    if (max(FLAT$group) < 2L) {
        ov.names <- vnames(FLAT, type = "ov")
        ov.names.y <- vnames(FLAT, type = "ov.nox")
        ov.names.x <- vnames(FLAT, type = "ov.x")
    }
    else {
        ov.names <- lapply(1:max(FLAT$group), function(x) vnames(FLAT,
            type = "ov", group = x))
        ov.names.y <- lapply(1:max(FLAT$group), function(x) vnames(FLAT,
            type = "ov.nox", group = x))
        ov.names.x <- lapply(1:max(FLAT$group), function(x) vnames(FLAT,
            type = "ov.x", group = x))
    }
    if (any(FLAT$op == "|")) {
        categorical <- TRUE
    ...
    if (!is.null(slotOptions)) {
        ...
        opt <- list(model = model, model.type = model.type, meanstructure = meanstructure,
            int.ov.free = int.ov.free, int.lv.free = int.lv.free,
            fixed.x = fixed.x, orthogonal = orthogonal, std.lv = std.lv,
            parameterization = parameterization, auto.fix.first = auto.fix.first,
            auto.fix.single = auto.fix.single, auto.var = auto.var,
            auto.cov.lv.x = auto.cov.lv.x, auto.cov.y = auto.cov.y,
            auto.th = auto.th, auto.delta = auto.delta, missing = missing,
            group = group, categorical = categorical, group.equal = group.equal,
            group.partial = group.partial, group.w.free = group.w.free,
            constraints = constraints, estimator = estimator,
            likelihood = likelihood, link = link, sample.cov.rescale = sample.cov.rescale,
            information = information, se = se, test = test,
            bootstrap = bootstrap, mimic = mimic, zero.add = zero.add,
            zero.keep.margins = zero.keep.margins, representation = representation,
            do.fit = do.fit, verbose = verbose, warn = warn,
            debug = debug)
    ...
    if (lavoptions$estimator == "PML") {
        ...
        if (!is.null(slotData)) {
            ov.types <- slotData@ov$type[slotData@ov$name %in%
                ovy]
        }
        else {
    ...
    if (!is.null(slotData)) {
        ...
        lavdata <- lavData(data = data, group = group, group.label = group.label,
            ov.names = ov.names, ordered = ordered, ov.names.x = ov.names.x,
            std.ov = std.ov, missing = lavoptions$missing, sample.cov = sample.cov,
            sample.mean = sample.mean, sample.nobs = sample.nobs,
    ...
    if (!is.null(slotParTable)) {
        ...
        lavpartable <- lavaanify(model = FLAT, meanstructure = lavoptions$meanstructure,
            int.ov.free = lavoptions$int.ov.free, int.lv.free = lavoptions$int.lv.free,
            orthogonal = lavoptions$orthogonal, fixed.x = lavoptions$fixed.x,
            std.lv = lavoptions$std.lv, parameterization = lavoptions$parameterization,
            constraints = constraints, auto.fix.first = lavoptions$auto.fix.first,
            ...
            group.equal = lavoptions$group.equal, group.partial = lavoptions$group.partial,
            group.w.free = lavoptions$group.w.free, debug = lavoptions$debug,
            warn = lavoptions$warn, as.data.frame. = FALSE)
    }
    else if (is.list(model)) {
    ...
    if (!is.null(slotSampleStats)) {
        ...
        lavsamplestats <- lav_samplestats_from_data(lavdata = lavdata,
            missing = lavoptions$missing, rescale = (lavoptions$estimator ==
                "ML" && lavoptions$likelihood == "normal"), estimator = lavoptions$estimator,
            ...
            group.w.free = lavoptions$group.w.free, missing.h1 = (lavoptions$missing !=
                "listwise"), WLS.V = WLS.V, NACOV = NACOV, ridge = ridge,
            zero.add = lavoptions$zero.add, zero.keep.margins = lavoptions$zero.keep.margins,
            debug = lavoptions$debug, verbose = lavoptions$verbose)
    }
    else if (lavdata@data.type == "moment") {
        lavsamplestats <- lav_samplestats_from_moments(sample.cov = sample.cov,
            sample.mean = sample.mean, sample.nobs = sample.nobs,
            ov.names = lavpta$vnames$ov, estimator = lavoptions$estimator,
            mimic = lavoptions$mimic, meanstructure = lavoptions$meanstructure,
            group.w.free = lavoptions$group.w.free, WLS.V = WLS.V,
            NACOV = NACOV, ridge = ridge, rescale = lavoptions$sample.cov.rescale)
    }
    else {
        ...
        lavsamplestats <- new("lavSampleStats", ngroups = lavdata@ngroups,
            nobs = as.list(rep(0L, lavdata@ngroups)), cov.x = vector("list",
                length = lavdata@ngroups), th.idx = th.idx, missing.flag = FALSE)
    }
    ...
    if (!is.null(slotModel)) {
        ...
        lavaanStart <- lav_start(start.method = start, lavpartable = lavpartable,
            lavsamplestats = lavsamplestats, model.type = lavoptions$model.type,
            mimic = lavoptions$mimic, debug = lavoptions$debug)
        timing$Start <- (proc.time()[3] - start.time)
        start.time <- proc.time()[3]
        lavmodel <- lav_model(lavpartable = lavpartable, start = lavaanStart,
            representation = lavoptions$representation, th.idx = lavsamplestats@th.idx,
            parameterization = lavoptions$parameterization, link = lavoptions$link,
            control = control, debug = lavoptions$debug)
        timing$Model <- (proc.time()[3] - start.time)
        start.time <- proc.time()[3]
    ...
    if (lavoptions$estimator == "PML") {
        ...
        for (g in 1:lavdata@ngroups) {
            ...
            LONG <- LongVecInd(no.x = nvar, all.thres = TH[[g]],
                index.var.of.thres = th.idx)
            lavcache[[g]] <- list(bifreq = bifreq, nobs = nobs,
                LONG = LONG)
        }
    }
    ...
    if (lavoptions$estimator == "MML") {
        ...
        for (g in 1:lavdata@ngroups) {
            nfac <- lavpta$nfac[[g]]
            lavcache[[g]]$GH <- lav_gauss_hermite_xw_dnorm(n = nGH,
                revert = FALSE, ndim = nfac)
        }
    }
    x <- NULL
    if (do.fit && lavoptions$estimator != "none" && lavmodel@nx.free >
        0L) {
        if (lavdata@data.type == "full" && length(unique(lavpartable$lhs[lavpartable$op ==
            "~"])) == 1L && length(vnames(lavpartable, "lv")) ==
            0L && sum(nchar(FLAT$fixed)) == 0 && !categorical &&
            !lavmodel@eq.constraints && length(lavdata@X) > 0L &&
            lavdata@ngroups == 1L && lavoptions$fixed && lavoptions$missing ==
            "listwise") {
            ov.y.idx <- match(vnames(lavpartable, "ov.y"), lavdata@ov.names[[1L]])
            ov.x.idx <- match(vnames(lavpartable, "ov.x"), lavdata@ov.names[[1L]])
            YX <- lavdata@X[[1L]]
            if (sum(length(lavmodel@x.ceq.idx) + length(lavmodel@x.cin.idx)) ==
                0L) {
                out <- lm.fit(x = cbind(1, YX[, ov.x.idx]), y = YX[,
                  ov.y.idx])
                x.beta <- out$coefficients
                y.rvar <- sum(out$residuals^2)/length(out$residuals)
                ...
                lavmodel <- lav_model_set_parameters(lavmodel,
                  x = x, estimator = lavoptions$estimator)
                attr(x, "iterations") <- 1L
                attr(x, "converged") <- TRUE
                attr(x, "control") <- control
                FX <- try(lav_model_objective(lavmodel = lavmodel,
                  lavsamplestats = lavsamplestats, estimator = lavoptions$estimator),
                  silent = TRUE)
            ...
            else if (lav_constraints_check_linear(lavmodel) ==
                TRUE) {
                A.ceq <- A.cin <- matrix(0, lavmodel@nx.free,
                  0)
                ...
                rvar.idx <- lavpartable$unco[lavpartable$op ==
                  "~~" & lavpartable$unco]
                if (lavoptions$meanstructure) {
                  int.idx <- lavpartable$unco[lavpartable$op ==
                    "~1" & lavpartable$unco]
                  A <- rbind(A[int.idx, , drop = FALSE], A[-c(int.idx,
                    rvar.idx), , drop = FALSE])
                }
                else {
                  A <- rbind(rep(0, ncol(A)), A[-c(rvar.idx),
                    , drop = FALSE])
                }
                ...
                out <- solve.QP(Dmat = X.X, dvec = X.Y, Amat = A,
                  bvec = rep(0, NCOL(A)), meq = length(lavmodel@x.ceq.idx))
                x.beta <- out$solution
                residuals <- Y - (X %*% x.beta)
                y.rvar <- sum(residuals^2)/length(residuals)
                if (!lavoptions$meanstructure) {
                  x <- numeric(1L + length(x.beta) - 1L)
                ...
                attr(x, "converged") <- TRUE
                attr(x, "control") <- control
                attr(x, "fx") <- lav_model_objective(lavmodel,
                  lavsamplestats = lavsamplestats, estimator = lavoptions$estimator)
                attr(con.jac, "inactive.idx") <- integer(0)
                attr(con.jac, "cin.idx") <- seq_len(ncol(A.cin)) +
                  ncol(A.ceq)
                attr(con.jac, "ceq.idx") <- seq_len(ncol(A.ceq))
                attr(x, "con.jac") <- con.jac
                con.lambda <- rep(1, nrow(con.jac))
            ...
            x <- lav_model_estimate(lavmodel = lavmodel, lavsamplestats = lavsamplestats,
                lavdata = lavdata, lavoptions = lavoptions, lavcache = lavcache)
    ...
    start.time <- proc.time()[3]
    VCOV <- NULL
    if (lavoptions$se != "none" && lavmodel@nx.free > 0L && attr(x,
        "converged")) {
        if (verbose)
    ...
    if (lavoptions$test != "none" && attr(x, "converged")) {
        ...
        TEST <- lav_model_test(lavmodel = lavmodel, lavpartable = lavpartable,
            ...
            x = x, VCOV = VCOV, lavdata = lavdata, lavcache = lavcache)
        if (verbose)
            cat(" done.\n")
    }
    else {
    ...
    start.time <- proc.time()[3]
    lavfit <- lav_model_fit(lavpartable = lavpartable, start = lavaanStart,
        lavmodel = lavmodel, x = x, VCOV = VCOV, TEST = TEST)
    timing$total <- (proc.time()[3] - start.time0)
    if (attr(x, "converged")) {
        var.idx <- which(lavpartable$op == "~~" & !lavpartable$lhs %in%
            unlist(lavpta$vnames$ov.ord) & lavpartable$lhs ==
            lavpartable$rhs)
        if (length(var.idx) > 0L && any(lavfit@est[var.idx] <
            0))
            warning("lavaan WARNING: some estimated variances are negative")
        if (length(vnames(lavpartable, type = "lv.regular")) >
            0L) {
            ETA <- computeVETA(lavmodel, lavsamplestats = lavsamplestats)
            for (g in 1:lavdata@ngroups) {
                txt.group <- ifelse(lavdata@ngroups > 1L, paste("in group",
                  g, ".", sep = ""), "")
                eigvals <- eigen(ETA[[g]], symmetric = TRUE,
                  only.values = TRUE)$values
                if (any(eigvals < -1 * .Machine$double.eps^(3/4)))
                  warning("lavaan WARNING: covariance matrix of latent variables is not positive definite;",
                    txt.group, " use inspect(fit,\"cov.lv\") to investigate.")
            }
        }
        ...
        for (g in 1:lavdata@ngroups) {
            ...
            if (length(num.idx) > 0L) {
                txt.group <- ifelse(lavdata@ngroups > 1L, paste("in group",
                  g, ".", sep = ""), "")
                eigvals <- eigen(THETA[[g]][num.idx, num.idx,
                  drop = FALSE], symmetric = TRUE, only.values = TRUE)$values
                if (any(eigvals < -1 * .Machine$double.eps^(3/4))) {
                  warning("lavaan WARNING: residual covariance matrix is not positive definite;",
                    txt.group, " use inspect(fit,\"cov.ov\") to investigate.")
                }
            }
        }
    }
    lavaan <- new("lavaan", call = mc, timing = timing, Options = lavoptions,
        ParTable = lavpartable, pta = lavpta, Data = lavdata,
        SampleStats = lavsamplestats, Model = lavmodel, Cache = lavcache,
        Fit = lavfit)
    lavaan
}
